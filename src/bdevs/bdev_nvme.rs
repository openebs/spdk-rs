...
use crate::{
    ...
    libspdk::{spdk_bdev_nvme_delete, spdk_nvme_path_id, SPDK_NVME_TRANSPORT_PCIE},
};

/// Async wrapper for `spdk_bdev_nvme_delete`.
/// `spdk_bdev_nvme_delete` differs from other bdev_*_delete function family,
...
pub async fn bdev_nvme_delete_async(
    name: &str,
    path_id: Option<spdk_nvme_path_id>,
    ...
    // `spdk_bdev_nvme_delete` failed to run: callback won't be called.
    if errno < 0 {
        drop_cb_arg::<()>(arg);
