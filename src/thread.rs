...
impl Thread {
    ...
    /// Moves the thread `tid` off the reactor cores.
    ///
    /// The mask is seeded from `BASE_CPUSET` (the container's cpuset, see
    /// [`Thread::capture_base_cpuset`]), the reactor cores are cleared from it,
    /// and the result is applied to `tid`. A `tid` of `0` means the calling
    /// thread.
